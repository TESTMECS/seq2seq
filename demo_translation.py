#!/usr/bin/env python
"""
Script to translate sentences using the seq2seq model with language flags.
"""
...
def main():
    ...
    # Use fixed parameters from the saved model (obtained through inspection)
    hidden_dim = 64
    emb_dim = 32
    n_layers = 1
    src_vocab_size = 100
    ...
    # Ensure tokenizers have special tokens
    bos_token = "[BOS]"
    eos_token = "[EOS]"
    
    # Verify special tokens exist in tokenizers or add defaults
    if not src_tokenizer.token_to_id(bos_token):
        print(f"Warning: {bos_token} token not found in source tokenizer")
    if not src_tokenizer.token_to_id(eos_token):
        print(f"Warning: {eos_token} token not found in source tokenizer")
    if not tgt_tokenizer.token_to_id(bos_token):
        print(f"Warning: {bos_token} token not found in target tokenizer")
    if not tgt_tokenizer.token_to_id(eos_token):
        print(f"Warning: {eos_token} token not found in target tokenizer")
    
    # Print example token IDs from tokenizers
    print("\nTokenizer Example:")
    sample_text = "Hello world"
    src_tokens = src_tokenizer.encode(sample_text).tokens
    print(f"Source text: '{sample_text}' → Tokens: {src_tokens}")
